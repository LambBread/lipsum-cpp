/**
 * @file lipsumc/sfuncs.inl
 *
 * @brief S functions of lipsum-cpp C wrapper
 *
 * This defines the &quot;S&quot; variants of functions in lipsum-cpp's C
 * wrapper. This file is under the BSD Zero-Clause License.
 *
 * @copyright Copyright (c) 2026 LambBread
 *
 * @author LambBread from github.com
 */

#pragma once

#ifndef LIPSUMC_SFUNCS_INL
#    define LIPSUMC_SFUNCS_INL

template <typename... Args> static void Av2Destroy(bool del, Args&&... args)
{
    if (!del)
    {
        return;
    }

    (lpsm_ArgVec2Destroy(std::forward<Args>(args)), ...);
}

/*
 * BULK
 * ----------------
 */

extern "C" char* lpsm_GenerateSentencesS(int                sentCount,
                                         lpsm_ArgVec2Handle word,
                                         lpsm_ArgVec2Handle frag,
                                         bool               useLipsum,
                                         lpsm_SourceHandle  source,
                                         bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateSentences(sentCount,
                                    *LPSM_CPPIFY(word, lpsm::ArgVec2),
                                    *LPSM_CPPIFY(frag, lpsm::ArgVec2),
                                    useLipsum,
                                    *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word, frag);
    return ret;
}

extern "C" char* lpsm_GenerateParagraphsS(int                paraCount,
                                          lpsm_ArgVec2Handle word,
                                          lpsm_ArgVec2Handle frag,
                                          lpsm_ArgVec2Handle sent,
                                          bool               useLipsum,
                                          lpsm_SourceHandle  source,
                                          bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateParagraphs(paraCount,
                                     *LPSM_CPPIFY(word, lpsm::ArgVec2),
                                     *LPSM_CPPIFY(frag, lpsm::ArgVec2),
                                     *LPSM_CPPIFY(sent, lpsm::ArgVec2),
                                     useLipsum,
                                     *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word, frag);
    return ret;
}

/*
 * MISC
 * ----------------
 */

extern "C" char*
lpsm_GenerateURLS(lpsm_ArgVec2Handle word, lpsm_SourceHandle source, bool del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateURL(*LPSM_CPPIFY(word, lpsm::ArgVec2),
                              *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word);
    return ret;
}

extern "C" char* lpsm_GenerateSlugS(lpsm_ArgVec2Handle word,
                                    char               separator,
                                    lpsm_SourceHandle  source,
                                    bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateSlug(*LPSM_CPPIFY(word, lpsm::ArgVec2),
                               separator,
                               *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word);
    return ret;
}

/*
 * CORE
 * ----------------
 */

extern "C" char* lpsm_GenerateSentenceFragmentS(lpsm_ArgVec2Handle word,
                                                lpsm_SourceHandle  source,
                                                bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateSentenceFragment(*LPSM_CPPIFY(word, lpsm::ArgVec2),
                                           *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word);
    return ret;
}

extern "C" char* lpsm_GenerateSentenceS(lpsm_ArgVec2Handle word,
                                        lpsm_ArgVec2Handle frag,
                                        lpsm_SourceHandle  source,
                                        bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateSentence(*LPSM_CPPIFY(word, lpsm::ArgVec2),
                                   *LPSM_CPPIFY(frag, lpsm::ArgVec2),
                                   *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word, frag);
    return ret;
}

extern "C" char* lpsm_GenerateParagraphS(lpsm_ArgVec2Handle word,
                                         lpsm_ArgVec2Handle frag,
                                         lpsm_ArgVec2Handle sent,
                                         bool               useLipsum,
                                         lpsm_SourceHandle  source,
                                         bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateParagraph(*LPSM_CPPIFY(word, lpsm::ArgVec2),
                                    *LPSM_CPPIFY(frag, lpsm::ArgVec2),
                                    *LPSM_CPPIFY(sent, lpsm::ArgVec2),
                                    useLipsum,
                                    *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word, frag, sent);
    return ret;
}

extern "C" char* lpsm_GenerateTextS(lpsm_ArgVec2Handle word,
                                    lpsm_ArgVec2Handle frag,
                                    lpsm_ArgVec2Handle sent,
                                    lpsm_ArgVec2Handle para,
                                    bool               useLipsum,
                                    lpsm_SourceHandle  source,
                                    bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateText(*LPSM_CPPIFY(word, lpsm::ArgVec2),
                               *LPSM_CPPIFY(frag, lpsm::ArgVec2),
                               *LPSM_CPPIFY(sent, lpsm::ArgVec2),
                               *LPSM_CPPIFY(para, lpsm::ArgVec2),
                               useLipsum,
                               *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word, frag, sent, para);
    return ret;
}

/*
 * MARKDOWN
 * ----------------
 */

extern "C" char* lpsm_GenerateMarkdownHeaderS(int                level,
                                              lpsm_ArgVec2Handle word,
                                              bool               useHtml,
                                              lpsm_SourceHandle  source,
                                              bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateMarkdownHeader(level,
                                         *LPSM_CPPIFY(word, lpsm::ArgVec2),
                                         useHtml,
                                         *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word);
    return ret;
}

extern "C" char* lpsm_GenerateMarkdownEmphasisS(bool               isBold,
                                                lpsm_ArgVec2Handle word,
                                                lpsm_ArgVec2Handle frag,
                                                bool               useHtml,
                                                lpsm_SourceHandle  source,
                                                bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateMarkdownEmphasis(isBold,
                                           *LPSM_CPPIFY(word, lpsm::ArgVec2),
                                           *LPSM_CPPIFY(frag, lpsm::ArgVec2),
                                           useHtml,
                                           *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word, frag);
    return ret;
}

extern "C" char* lpsm_GenerateMarkdownLinkS(lpsm_ArgVec2Handle word,
                                            lpsm_ArgVec2Handle frag,
                                            lpsm_ArgVec2Handle wordURL,
                                            bool               useHtml,
                                            lpsm_SourceHandle  source,
                                            bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateMarkdownLink(*LPSM_CPPIFY(word, lpsm::ArgVec2),
                                       *LPSM_CPPIFY(frag, lpsm::ArgVec2),
                                       *LPSM_CPPIFY(wordURL, lpsm::ArgVec2),
                                       useHtml,
                                       *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word, frag, wordURL);
    return ret;
}

extern "C" char* lpsm_GenerateMarkdownListS(bool               ordered,
                                            lpsm_ArgVec2Handle word,
                                            lpsm_ArgVec2Handle frag,
                                            lpsm_ArgVec2Handle point,
                                            bool               useHtml,
                                            lpsm_SourceHandle  source,
                                            bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateMarkdownList(ordered,
                                       *LPSM_CPPIFY(word, lpsm::ArgVec2),
                                       *LPSM_CPPIFY(frag, lpsm::ArgVec2),
                                       *LPSM_CPPIFY(point, lpsm::ArgVec2),
                                       useHtml,
                                       *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word, frag, point);
    return ret;
}

extern "C" char* lpsm_GenerateMarkdownParagraphS(lpsm_ArgVec2Handle word,
                                                 lpsm_ArgVec2Handle frag,
                                                 lpsm_ArgVec2Handle sent,
                                                 lpsm_ArgVec2Handle wordFmt,
                                                 lpsm_ArgVec2Handle fragFmt,
                                                 lpsm_ArgVec2Handle wordLink,
                                                 bool               useLipsum,
                                                 bool               useHtml,
                                                 lpsm_SourceHandle  source,
                                                 bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateMarkdownParagraph(*LPSM_CPPIFY(word, lpsm::ArgVec2),
                                            *LPSM_CPPIFY(frag, lpsm::ArgVec2),
                                            *LPSM_CPPIFY(sent, lpsm::ArgVec2),
                                            *LPSM_CPPIFY(wordFmt,
                                                         lpsm::ArgVec2),
                                            *LPSM_CPPIFY(fragFmt,
                                                         lpsm::ArgVec2),
                                            *LPSM_CPPIFY(wordLink,
                                                         lpsm::ArgVec2),
                                            useLipsum,
                                            useHtml,
                                            *LPSM_CPPIFY(source,
                                                         lpsm::Source)));
    Av2Destroy(del, word, frag, sent, wordFmt, fragFmt, wordLink);
    return ret;
}

extern "C" char* lpsm_GenerateMarkdownParagraphsS(int                paraCount,
                                                  lpsm_ArgVec2Handle word,
                                                  lpsm_ArgVec2Handle frag,
                                                  lpsm_ArgVec2Handle sent,
                                                  lpsm_ArgVec2Handle wordFmt,
                                                  lpsm_ArgVec2Handle fragFmt,
                                                  lpsm_ArgVec2Handle wordLink,
                                                  bool               useLipsum,
                                                  bool               useHtml,
                                                  lpsm_SourceHandle  source,
                                                  bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateMarkdownParagraphs(paraCount,
                                             *LPSM_CPPIFY(word, lpsm::ArgVec2),
                                             *LPSM_CPPIFY(frag, lpsm::ArgVec2),
                                             *LPSM_CPPIFY(sent, lpsm::ArgVec2),
                                             *LPSM_CPPIFY(wordFmt,
                                                          lpsm::ArgVec2),
                                             *LPSM_CPPIFY(fragFmt,
                                                          lpsm::ArgVec2),
                                             *LPSM_CPPIFY(wordLink,
                                                          lpsm::ArgVec2),
                                             useLipsum,
                                             useHtml,
                                             *LPSM_CPPIFY(source,
                                                          lpsm::Source)));
    Av2Destroy(del, word, frag, sent, wordFmt, fragFmt, wordLink);
    return ret;
}

extern "C" char* lpsm_GenerateMarkdownTextS(lpsm_ArgVec2Handle word,
                                            lpsm_ArgVec2Handle frag,
                                            lpsm_ArgVec2Handle sent,
                                            lpsm_ArgVec2Handle point,
                                            lpsm_ArgVec2Handle wordFmt,
                                            lpsm_ArgVec2Handle fragFmt,
                                            lpsm_ArgVec2Handle wordHead,
                                            lpsm_ArgVec2Handle level,
                                            int                numElements,
                                            bool               useHtml,
                                            lpsm_SourceHandle  source,
                                            bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateMarkdownText(*LPSM_CPPIFY(word, lpsm::ArgVec2),
                                       *LPSM_CPPIFY(frag, lpsm::ArgVec2),
                                       *LPSM_CPPIFY(sent, lpsm::ArgVec2),
                                       *LPSM_CPPIFY(point, lpsm::ArgVec2),
                                       *LPSM_CPPIFY(wordFmt, lpsm::ArgVec2),
                                       *LPSM_CPPIFY(fragFmt, lpsm::ArgVec2),
                                       *LPSM_CPPIFY(wordHead, lpsm::ArgVec2),
                                       *LPSM_CPPIFY(level, lpsm::ArgVec2),
                                       numElements,
                                       useHtml,
                                       *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word, frag, sent, point, wordFmt, fragFmt, wordHead, level);
    return ret;
}

/*
 * FORMATS
 */

extern "C" char* lpsm_GenerateXMLDocumentS(int                choices,
                                           lpsm_ArgVec2Handle word,
                                           lpsm_ArgVec2Handle frag,
                                           lpsm_SourceHandle  source,
                                           bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateXMLDocument(choices,
                                      *LPSM_CPPIFY(word, lpsm::ArgVec2),
                                      *LPSM_CPPIFY(frag, lpsm::ArgVec2),
                                      *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, word, frag);
    return ret;
}

extern "C" char* lpsm_GenerateJSONObjectS(int                depth,
                                          int                maxDepth,
                                          lpsm_ArgVec2Handle jsonLength,
                                          lpsm_SourceHandle  source,
                                          bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateJSONObject(depth,
                                     maxDepth,
                                     *LPSM_CPPIFY(jsonLength, lpsm::ArgVec2),
                                     *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, jsonLength);
    return ret;
}

extern "C" char* lpsm_GenerateJSONArrayS(int                depth,
                                         int                maxDepth,
                                         lpsm_ArgVec2Handle jsonLength,
                                         lpsm_SourceHandle  source,
                                         bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateJSONArray(depth,
                                    maxDepth,
                                    *LPSM_CPPIFY(jsonLength, lpsm::ArgVec2),
                                    *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, jsonLength);
    return ret;
}

extern "C" char* lpsm_GenerateJSONValueS(int                depth,
                                         int                maxDepth,
                                         lpsm_ArgVec2Handle jsonLength,
                                         lpsm_SourceHandle  source,
                                         bool               del)
{
    char* ret = ConvertToCstr(
            lpsm::GenerateJSONValue(depth,
                                    maxDepth,
                                    *LPSM_CPPIFY(jsonLength, lpsm::ArgVec2),
                                    *LPSM_CPPIFY(source, lpsm::Source)));
    Av2Destroy(del, jsonLength);
    return ret;
}
#endif
